...
object CrashLogger {
    ...
    private fun currentProcessName(context: Context): String = runCatching {
        if (Build.VERSION.SDK_INT >= 28) android.app.Application.getProcessName() else context.packageName
    ...
    /** Only Imux's private runtime log. No system/vendor logcat is mixed into this file. */
    ...
    /** Best-effort live view of this process only; never requests global log access. */
    fun readProcessLogcat(): String = runCatching {
        val process = ProcessBuilder(
            "logcat", "-d", "--pid", android.os.Process.myPid().toString(), "-v", "threadtime"
    ...
    private fun trimIfNeeded(file: File) {
        if (file.length() <= MAX_CHARS) return
        val content = file.readText()
        file.writeText(content.takeLast(MAX_CHARS))
    }
}
